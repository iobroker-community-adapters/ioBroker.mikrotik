...
module.exports = function (grunt) {
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new mikrotik name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/mikrotik.png')) {
            fs.renameSync(__dirname + '/admin/mikrotik.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/mikrotik.html')) {
            fs.renameSync(__dirname + '/widgets/mikrotik.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/mikrotik/js/mikrotik.js')) {
            fs.renameSync(__dirname + '/widgets/mikrotik/js/mikrotik.js', __dirname + '/widgets/mikrotik/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/mikrotik')) {
            fs.renameSync(__dirname + '/widgets/mikrotik',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Mikrotik/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Mikrotik'
                        },
                        {
                            match: /instalator/g,
                            replacement: author
